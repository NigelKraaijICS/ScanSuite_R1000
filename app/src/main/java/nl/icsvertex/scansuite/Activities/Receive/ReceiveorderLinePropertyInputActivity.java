...
public class ReceiveorderLinePropertyInputActivity extends AppCompatActivity implements iICSDefaultActivity {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (item.getItemId() == android.R.id.home) {
            if (getQuantityAvailable() > 0 ) {
                mShowUnderReceiveDialog(cAppExtension.activity.getString(R.string.message_cancel_line), cAppExtension.activity.getString(R.string.message_accept_line));
                return true;
            }
    ...
    public  void pCancelReceive() {
        ...
        //cReceiveorderLine.currentReceiveorderLine.pUpdateProcessingSequenceBln("");
        cReceiveorderSummaryLine.currentReceiveorderSummaryLine.pResetRst();
    ...
    public  void pHandeManualAction(cBarcodeScan pvBarcodeScan){
        ...
        if (this.amountHandledBln){
            ...
            if (cReceiveorderSummaryLine.currentReceiveorderSummaryLine.specificLinePropertyObl(cLineProperty.currentLineProperty.getPropertyCodeStr()).size() > 1){
                ...
                int commitInt;
                double amountDbl = this.intakeorderBarcode.getQuantityHandledDbl() / availableInt;
                for(cLineProperty lineProperty : cReceiveorderSummaryLine.currentReceiveorderSummaryLine.specificLinePropertyObl(cLineProperty.currentLineProperty.getPropertyCodeStr())){
                    availableInt -= 1;

                   switch (lineProperty.propertyValueObl().size()){
                        case 0:
                           ...
                           lineProperty.pReceiveLineValueAdded(cLinePropertyValue.currentLinePropertyValue.getValueStr());
                           cLinePropertyValue.currentLinePropertyValue.quantityDbl = commitInt;
                           break;
                       ...
                       case 1:
                           ...
                           cLinePropertyValue.currentLinePropertyValue = lineProperty.propertyValueObl().get(0);
                           cLinePropertyValue.currentLinePropertyValue.quantityDbl = commitInt;
                           break;
                       default:
                           int loopCounterInt = lineProperty.propertyValueObl().size();
                           int totalCommitInt = (int) Math.round(amountDbl);

                           totalInt -= totalCommitInt;
                           for (cLinePropertyValue propertyValue: lineProperty.propertyValueObl()){
                               loopCounterInt -= 1;

                               commitInt = (int) Math.round(amountDbl/ lineProperty.propertyValueObl().size());
                               totalCommitInt -= commitInt;
                               if (loopCounterInt == 0){
                                   commitInt += totalCommitInt;
                                   if (availableInt == 0) {
                                       commitInt += totalInt;
                                   }
                               }
                               propertyValue.quantityDbl = commitInt;
                           }
                           break;
                   }
                }
    ...
    public void pHandled() {
        ReceiveLinesActivity.handledViaPropertysBln = true;
    ...
    private void mSetCloseListener() {
        this.imageButtonDone.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (getQuantityAvailable() > 0 && !amountHandledBln ) {
                    mShowUnderReceiveDialog(cAppExtension.activity.getString(R.string.message_cancel_line), cAppExtension.activity.getString(R.string.message_accept_line));
                    return;
                }
        ...
        this.itemPropertyTabLayout.addOnTabSelectedListener(new TabLayout.OnTabSelectedListener() {
            ...
            public void onTabSelected(TabLayout.Tab tab) {
                if (tab.getPosition() > 0){
                    if (getQuantityAvailable() > 0) {
                        mShowUnderReceivePropertyDialog(cAppExtension.activity.getString(R.string.message_cancel), cAppExtension.activity.getString(R.string.message_accept));
    ...
    private  void mShowUnderReceiveDialog(String pvRejectStr, String pvAcceptStr) {
        ...
        final AcceptRejectFragment acceptRejectFragment = new AcceptRejectFragment(cAppExtension.activity.getString(R.string.message_underpick_header),
                cAppExtension.activity.getString(R.string.message_underreceive_text,
                        cText.pDoubleToStringStr(cReceiveorderSummaryLine.currentReceiveorderSummaryLine.getQuantityDbl()),
    ...
    private  void mShowUnderReceivePropertyDialog(String pvRejectStr, String pvAcceptStr) {
        ...
        final AcceptRejectPropertyFragment acceptRejectPropertyFragment = new AcceptRejectPropertyFragment(cAppExtension.activity.getString(R.string.message_underpick_header),
                cAppExtension.activity.getString(R.string.message_underreceive_property_text,
                        cText.pDoubleToStringStr(cReceiveorderSummaryLine.currentReceiveorderSummaryLine.getQuantityDbl()),
                        cText.pDoubleToStringStr(this.getQuantityHandledDbl())),
                pvRejectStr,
                pvAcceptStr );
    ...
    private void mShowNumberPickerFragment() {
        ...
        double availableDbl  = cReceiveorderSummaryLine.currentReceiveorderSummaryLine.getQuantityDbl();
